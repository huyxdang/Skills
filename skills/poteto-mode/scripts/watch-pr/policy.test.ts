...
describe("readiness truth table", () => {
  it("fails closed unless GitHub reports a clean merge state", () => {
  ...
  it("blocks a pull request while an approval is required", async () => {
    const reader = fakeReader({ facts: { reviewDecision: "REVIEW_REQUIRED" } });
  ...
  it("blocks a pull request while mergeability is unknown", async () => {
    const reader = fakeReader({ facts: { mergeable: "UNKNOWN" } });
    const snapshot = await readSnapshot({
      reader,
      context: context(1),
      pendingHistory: "include",
    ...
    expect(classifyPr(snapshot)).toMatchObject({
      kind: "blocker",
      blocker: { kind: "merge-gate", reason: "mergeability-unknown" },
    });
  });
