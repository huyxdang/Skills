...
it("returns the next review-thread cursor", () => {
  ...
  expect(page).toEqual({ threads: [], endCursor: "thread-page-2" });
});

it("fails closed when a review-thread page omits its next cursor", () => {
  expect(() =>
    parseReviewThreadPage({
      data: {
        repository: {
